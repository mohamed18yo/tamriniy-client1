...
const Orders = () => {
  const { getOrders, deleteOrder, deleteAllOrder, addOrder, editOrder } =
    useOrdersHook();
  const [orders, setOrders] = useState([]);
  const [addBoxVisible, setAddBoxVisible] = useState(false);
  const [editBoxVisible, setEditBoxVisible] = useState(false);
  const [deleteBoxVisible, setDeleteBoxVisible] = useState(false);
  const [deleteAllBoxVisible, setDeleteAllBoxVisible] = useState(false);
  ...
  return (
    <>
      ...
      /> */}
      <DeleteBox
        visible={deleteAllBoxVisible}
        setVisible={setDeleteAllBoxVisible}
        title={`حذف كل الطلبات`}
        onDelete={async () => {
          if (await deleteAllOrder()) {
            setOrders([]);
            setDeleteAllBoxVisible(false);
          }
        }}
      />
      <DataBox
        ...
        inputs={[
          ...
          },
          {
            ...
            props: {
              ...
              name: "statusId",
              onChange: (e) =>
                setOrderObj({ ...orderObj, statusId: e.target.value }),
            },
            options: [
              {
                label: "لم يتم الدفع",
